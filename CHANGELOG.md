...
### Changed
- **定价参考基准切换**：改用 eBay/Temu 竞品清仓底价作为参考，确保转售商有稳定利润空间
- 倒推公式调整：`B_max = SP × 0.55 − 固定成本`（扣除平台佣金 15% + 目标利润 30%）
...
### Added
- **多平台比价**：估价参考从 TikTok/Whatnot 扩展到 eBay + Temu，覆盖更广泛的二手市场价格数据
...
### Fixed
- 下调收货估价：主收货率 25% → 18%，快速出货率 20% → 13%，更贴近实际市场回收水平
...
### Changed
- **全局货币单位切换为美元 $**：所有估价输出、UI 展示统一使用 USD
...
### Fixed
- 修复价格区间解析错误："¥28-33" 被解析为 ¥2,833 的 bug，现在正确返回区间
...
### Changed
- 定价模型重构：引入**倒推定价法**，以竞品市场价为起点，逐层倒推最高可收货价，取代原有主观估价逻辑
...
### Added
- 单元测试套件：`parsePrice`、`parseJson`、`findByKeywords` 覆盖率达 97%
...
### Changed
- **AI 服务商切换**：从 Kimi (Moonshot) 切换到 OpenAI（定价对话使用 gpt-4o-mini，图像识别保留 Kimi Vision）
...
### Added
- 回收流程管理模块（`/recovery`）：支持待回收商品跟踪和状态流转
